...
H_main  = 0.3;  
L_main  = 1.5;  
//Mesh.MeshSizeMin = 0.00001; // Close to your hwall_n
//Mesh.MeshSizeMax = 0.01;   // Prevents huge cells in the far-field
// --- Points ---
Point(1) = {0, 0, 0};                   
Point(2) = {0, H_inlet, 0};             
...
//Point(9) = {0, H_inlet + H_inlet + H_inlet + H_inlet + H_inlet + H_inlet, 0};  
// Point(10) = {L_inlet + L_main, H_inlet + H_inlet + H_inlet + H_inlet + H_inlet + H_inlet, 0 }; 
...
// Vertical: Inlet(1) + Step(3) must equal Outlet(5)
 ...
 Transfinite Curve {3} = 150 Using Progression 1.04;  
 Transfinite Curve {7} = 150 Using Progression 1.04;  
 Transfinite Curve {5} = 150 Using Progression 1.04;  

 Transfinite Curve {4} = 1400 Using Progression 1.003;

 Transfinite Curve {1} = 15;
 Transfinite Curve {9} = 15;
 Transfinite Curve {10} = 1400 Using Progression 1.003;
 Transfinite Curve {6} = 1400 Using Progression 1.003;
